...
std::vector<uint8_t> extractUdsPayload(const std::vector<uint8_t>& frameData) {
    if (frameData.empty()) return {};
    const auto frameType = static_cast<uint8_t>(frameData[0] & 0xF0);
    if (frameType == 0x00) {
        const auto len = static_cast<size_t>(frameData[0] & 0x0F);
        if (frameData.size() < len + 1) return {};
        return {frameData.begin() + 1, frameData.begin() + 1 + static_cast<std::ptrdiff_t>(len)};
    }
    // For this deterministic mock, only single-frame requests are converted to ECU responses.
    // Multi-frame request handling can be added by collecting FF/CF frames per channel.
    return frameData;
}
...
long J2534MockApi::WriteMsgs(unsigned long channelId, const J2534Msg* msgs, unsigned long* numMsgs, unsigned long /*timeoutMs*/) {
    ...
    const auto requestPayload = extractUdsPayload(msgs[0].data);
    const auto responsePayload = buildUdsResponse(requestPayload);
    transport::IsoTpEngine isotp;
    const auto responseFrames = isotp.segmentForTransmit(responsePayload);
    for (const auto& frame : responseFrames) {
        J2534Msg response{};
